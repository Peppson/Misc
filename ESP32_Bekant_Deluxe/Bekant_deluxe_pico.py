...
knapp_1 = machine.Pin(14, machine.Pin.IN, Pin.PULL_DOWN) 
knapp_2 = machine.Pin(13, machine.Pin.IN, Pin.PULL_DOWN)
knapp_3 = machine.Pin(12, machine.Pin.IN, Pin.PULL_DOWN)
knapp_4 = machine.Pin(15, machine.Pin.IN, Pin.PULL_DOWN)

gpio_up = machine.Pin(27, machine.Pin.OUT, Pin.PULL_DOWN)    #27 (off = 1)
gpio_down = machine.Pin(4, machine.Pin.OUT, Pin.PULL_DOWN)   #4  (off = 2)
gpio_up.value(0)
gpio_down.value(0)
...
# Oled display init
i2c = I2C(0, scl=Pin(21), sda=Pin(20), freq=const(400000))
    ...
    # endast 1 knapp tryckt?
        ...
        for x in range(54):
            if knapp1.value() == 1 or knapp2.value() == 1:
                ...
                if len(avg_list) >= 53:
                    avg_list.pop(0)
                if x > 3 and oled_display != 0: # forsätt visa sensor på skärm kort tid efter knapp har släppts
                    oled_display = 0 #TODO
...
# Main Core. Polling istället för interrupt, mkt bättre
def main_core():
    global interrupt_off
    while True:
        try:
            # for i in range(for x in range(nr)):
            for i in range (50):
                for x in range(const(5000)):
...
def core1_grid_moving(nr):
    ...
    while oled_display == nr:
        ...
        # nr i cm
            ...
            write20.text("   " + str(math_sensor), 22, 18) #22 18 
        else:
            write20.text(str(math_sensor), 23, 18)         #22 18    
        # bar down
        if nr == 10:
...
def core1_höjd_current():
    ...
    if math_sensor < const(100):
        write20.text("   " + str(math_sensor) + " cm  ", 22, 18)
    else:
        write20.text(str(math_sensor) + " cm  ", 23, 18)
